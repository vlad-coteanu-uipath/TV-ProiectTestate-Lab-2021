...
public class Problem_solve_SDL_11_Test {
    ...
    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_solve_SDL_11_equivalence_partitioning() {
        return prepareDynamicTestData(testDataProvider.getEquivalencePartitioningTestData().stream()
                .map(p -> new Pair<Problem, Result>(new Problem_solve_SDL_11(p.getKey()), p.getValue())).collect(Collectors.toList()));
    }

    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_solve_SDL_11_boundary_values() {
        return prepareDynamicTestData(testDataProvider.getBoundaryAnalysisTestData().stream()
                .map(p -> new Pair<Problem, Result>(new Problem_solve_SDL_11(p.getKey()), p.getValue())).collect(Collectors.toList()));
    }

    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_solve_SDL_11_cause_effect_graph() {
