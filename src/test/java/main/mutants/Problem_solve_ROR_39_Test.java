...
public class Problem_solve_ROR_39_Test {
    ...
    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_solve_ROR_39_equivalence_partitioning() {
        return prepareDynamicTestData(testDataProvider.getEquivalencePartitioningTestData().stream()
                .map(p -> new Pair<Problem, Result>(new Problem_solve_ROR_39(p.getKey()), p.getValue())).collect(Collectors.toList()));
    }

    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_solve_ROR_39_boundary_values() {
        return prepareDynamicTestData(testDataProvider.getBoundaryAnalysisTestData().stream()
                .map(p -> new Pair<Problem, Result>(new Problem_solve_ROR_39(p.getKey()), p.getValue())).collect(Collectors.toList()));
    }

    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_solve_ROR_39_cause_effect_graph() {
        return prepareDynamicTestData(testDataProvider.getCauseEffectGraphingTestData().stream()
                .map(p -> new Pair<Problem, Result>(new Problem_solve_ROR_39(p.getKey()), p.getValue())).collect(Collectors.toList()));
    }

    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_solve_ROR_39_kill_mutant() {
        return prepareDynamicTestData(testDataProvider.getAdditionalTestDataToKillMutants().stream()
                .map(p -> new Pair<Problem, Result>(new Problem_solve_ROR_39(p.getKey()), p.getValue())).collect(Collectors.toList()));
    }
