...
public class Problem_mirror_AORB_20_Test {
    ...
    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_mirror_AORB_20_equivalence_partitioning() {
        return prepareDynamicTestData(testDataProvider.getEquivalencePartitioningTestData().stream()
                .map(p -> new Pair<Problem, Result>(new Problem_mirror_AORB_20(p.getKey()), p.getValue())).collect(Collectors.toList()));
    }

    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_mirror_AORB_20_boundary_values() {
        return prepareDynamicTestData(testDataProvider.getBoundaryAnalysisTestData().stream()
                .map(p -> new Pair<Problem, Result>(new Problem_mirror_AORB_20(p.getKey()), p.getValue())).collect(Collectors.toList()));
    }

    @TestFactory
    public Iterable<DynamicTest> mutant_Problem_mirror_AORB_20_cause_effect_graph() {
